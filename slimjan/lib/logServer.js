...
function logServer(){
	this._winston = new (winston.Logger)({
		transports: [
             ...
             new winston.transports.File({ filename: 'slimJan.log', handleExceptions: true, timestamp: true })
         ],
         exitOnError: false
	});
...
logServer.prototype.logHttpReq = function logHttpReq(req){
	this._winston.info('HTTP ' + req.method + ' Request From ' + req.connection.remoteAddress + ' to ' + req.url + ': ' + JSON.stringify(req.body));
}
