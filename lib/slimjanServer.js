        ...
        logger.error('AMQP: ' + xerr);
				...
				async.parallel({
					sense: function (callback){
						helpers.getRules(data.d_hw, 'sense', function cbSense(err, info){
							callback(err,info);
						});
					},
					rules: function (callback){
						helpers.getRules(data.d_hw, 'rules', function cbRules(err, info){
							callback(err,info);
						});
					},
					probe: function (callback){
						helpers.getRules(data.d_hw, 'probe', function cbProbe(err, info){
							callback(err,info);
						});
					}
				},
