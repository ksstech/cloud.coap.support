...
var events = require('./eventLog.js');
...
function logHttpRequest(req,res){
    ...
    var msg = '[HTTP][Request][' + date.toDateString() + '][' + date.toTimeString() + ']';
        ...
        console.log(msg);
        if('request_id' in res._headers) req.messageId = res._headers['request_id'] || uuid.v1();
        else req.messageId = uuid.v1();
        res.messageId = req.messageId;
        events.logHttpRequest(req);
        events.logHttpResponse(res);
}

function logHttpObserveStart(req){
    var date = new Date();
    var msg = '[HTTP][Observe][Start][' + date.toDateString() + '][' + date.toTimeString() + ']';
        msg += ' ' + req.route.method;
        msg += ' ' + req.route.path;
        msg += ' FROM: ' + getRemoteHttpIp(req);
        console.log(msg);
        req.messageId = uuid.v1();
        events.logHttpRequest(req);
}

function logHttpObserveEnd(req,res){
    var date = new Date();
    var msg = '[HTTP][Observe][End][' + date.toDateString() + '][' + date.toTimeString() + ']';
        msg += ' ' + req.route.method;
        msg += ' ' + req.route.path;
        msg += ' FROM: ' + getRemoteHttpIp(req);
        console.log(msg);
...
module.exports = {
    ...
    logHttpObserveStart: logHttpObserveStart,
    logHttpObserveEnd: logHttpObserveEnd
};