      ...
      conn.createChannel(function(err,channel){
        ...
        if(err) that.logger.error('AMQP: ' + err);
          ...
          channel.consume(that.returnRegister, function(msg){
            if (msg !== null) {
              logger.info('AMQP REC ' + that.returnRegister + ': '+  message.toString());
              that.emit(that.returnRegister, JSON.parse(message.toString()));
            }
          });
        }
      });
