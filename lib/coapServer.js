...
function handelRegisterResponse(incoming, mqtt, amqp){
...
function handleFeedbackResponse (incoming, mqtt, amqp){
	if(incoming.packet.confirmable) {
		sendACK(incoming, function sendACKCB(err){
			if (err) logger.error(err);
		});
...
function coapServer(log, mqtt, amqp){
    ...
    server.on('message', function(data, rinfo) {
      ...
      case 'RER':
    	  ...
    	  handelRegisterResponse(req, mqtt, amqp);
    	  break;
      case 'SER':
    	  //do sense response
      ...
      case 'FDB':
    	  //do feedback stuff
    	  handleFeedbackResponse(req,mqtt, amqp)
    	  break;
      default:
    	  //UKN stuff
    ...
    amqp.amqpChannel.on('imacs.slimjan.register.queue', function(data){
  		postInstructions(data.content.toString());
  	});
