...
function handlePingRequest(incoming){
	...
	logger.info('CoAP PNG  from: ' + incoming.source.address + ':' + incoming.source.port);
	//send ack
...
function handelRegisterResponse(incoming, amqp){
  ...
  msgRegisterInFlight[uid].reset();
	...
	//if(incoming.packet.options['Content-Format'])
      ...
      //MWP 20160402 For testing purposes took out the tests for these
...
function postInstructions(data){
	...
	if(!data.d_id) logger.error('AMQP: slimjan.rules: no d_id');
      ...
      //logger.info(mote);
            ...
            if(data.rules.probe && data.rules.probe.length > 0){
...
function handleSenseResponse(incoming){
  ...
  msgSenseInFlight[uid].reset();
	delete msgSenseInFlight[uid];
	return logger.info('CoAP sense from: ' + incoming.source.address + ':' + incoming.source.port + ': ' + incoming.packet.code);
}
...
function handleRuleResponse(incoming){
  ...
  msgRuleInFlight[uid].reset();
	delete msgRuleInFlight[uid];
	return logger.info('CoAP rule from: ' + incoming.source.address + ':' + incoming.source.port + ': ' + incoming.packet.code);
}
...
function handleProbeResponse(incoming){
  ...
  msgProbeInFlight[uid].reset();
	delete msgProbeInFlight[uid];
	return logger.info('CoAP probe from: ' + incoming.source.address + ':' + incoming.source.port + ': ' + incoming.packet.code);
}
...
function handleFeedbackResponse (incoming, amqp){
  ...
  //handle missing d_id
  var data = JSON.parse(incoming.packet.payload);
  //logger.info(data);
  if(!data['d_id']) {
    var bn = data['bn'];
    var arrbn = bn.split(':');
  ...
  //might be an alert then publish as alert
  if(data['cat'] === 'sense') amqp.publishSense(JSON.stringify(data));
  else amqp.publishAlert(JSON.stringify(data));
